...
tdata = xr.open_dataset(seed_dir + "/" + month + "T.nc", chunks={"x":50, "y":50})
hgriddata = xr.open_dataset(seed_dir + "/mesh_hgr.nc" , chunks={"x":50, "y":50})
zgriddata = xr.open_dataset(seed_dir + "/mesh_zgr.nc" , chunks={"x":50, "y":50})
maskdata = xr.open_dataset(seed_dir + "/mask.nc", chunks={"x":50, "y":50})
...
for ipin, jpin in zip(ipins, jpins):
    ...
    for depth in depths:
        ...
        rho_ver = 1/veronis_density(so_cube[:,jpin,ipin].to_numpy(), to_cube[:,jpin,ipin].to_numpy(), deptht_cube[:,jpin,ipin].to_numpy(), depth, eos=eos, p_ref=ver_ref)
        ...
        plt.pcolormesh(z.to_numpy(), cmap=cmocean.cm.haline_r, vmin=0, vmax=depth_cube.max().to_numpy())
        plt.colorbar()
